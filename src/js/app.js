...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const weightData = {
        ...
        12:{weight:800,model:'DT-800',range:'10-14 mm',freq:'8-12 Hz',power:'4.2 kW'},
        13:{weight:840,model:'DT-840',range:'11-15 mm',freq:'8-11 Hz',power:'4.5 kW'},
        14:{weight:880,model:'DT-880',range:'12-16 mm',freq:'7-10 Hz',power:'4.8 kW'},
        15:{weight:920,model:'DT-920',range:'13-17 mm',freq:'7-9 Hz',power:'5.0 kW'},
        16:{weight:960,model:'DT-960',range:'14-18 mm',freq:'6-9 Hz',power:'5.2 kW'},
        17:{weight:1000,model:'DT-1000',range:'15-19 mm',freq:'6-8 Hz',power:'5.5 kW'},
        18:{weight:1040,model:'DT-1040',range:'16-20 mm',freq:'5-8 Hz',power:'5.8 kW'},
        19:{weight:1080,model:'DT-1080',range:'17-21 mm',freq:'5-7 Hz',power:'6.0 kW'},
        20:{weight:1120,model:'DT-1120',range:'18-22 mm',freq:'5-7 Hz',power:'6.2 kW'},
        21:{weight:1160,model:'DT-1160',range:'19-23 mm',freq:'4-6 Hz',power:'6.5 kW'},
        22:{weight:1200,model:'DT-1200',range:'20-24 mm',freq:'4-6 Hz',power:'6.8 kW'},
        23:{weight:1240,model:'DT-1240',range:'21-25 mm',freq:'4-5 Hz',power:'7.0 kW'},
        24:{weight:1280,model:'DT-1280',range:'22-26 mm',freq:'3-5 Hz',power:'7.2 kW'},
        25:{weight:1320,model:'DT-1320',range:'23-27 mm',freq:'3-5 Hz',power:'7.5 kW'},
        26:{weight:1360,model:'DT-1360',range:'24-28 mm',freq:'3-4 Hz',power:'7.8 kW'},
        27:{weight:1400,model:'DT-1400',range:'25-29 mm',freq:'3-4 Hz',power:'8.0 kW'},
        28:{weight:1440,model:'DT-1440',range:'26-30 mm',freq:'2-4 Hz',power:'8.2 kW'},
        29:{weight:1480,model:'DT-1480',range:'27-30 mm',freq:'2-3 Hz',power:'8.5 kW'},
        30:{weight:1520,model:'DT-1520',range:'28-30 mm',freq:'2-3 Hz',power:'8.8 kW'}
    };
    const iceTypeFactors = { rime:0.85, glaze:1.0, mixed:1.15, hard:1.3 };

    function calcWeight(th,type){
        const b=weightData[th]||weightData[5];
        const w=Math.round(b.weight*(iceTypeFactors[type]||1.0));
        return {weight:w,model:'DT-'+w,range:b.range,freq:b.freq,power:b.power};
    }

    function updWeight(){
        const th=parseInt(document.getElementById('iceThickness').value);
        const type=document.getElementById('iceType').value;
        const r=calcWeight(th,type);
        document.getElementById('weightResult').textContent=r.weight;
        document.getElementById('bladeModel').textContent=r.model;
        document.getElementById('iceRange').textContent=r.range;
        document.getElementById('impactFreq').textContent=r.freq;
        document.getElementById('powerSuggest').textContent=r.power;
        drawWeightChart(th,r.weight);
    }

    function drawWeightChart(ct,cw){
        const c=document.getElementById('weightChart'),ctx=c.getContext('2d');
        const w=c.width,h=c.height,pt=20,pr=20,pb=30,pl=40,cw2=w-pl-pr,ch=h-pt-pb;
        ctx.clearRect(0,0,w,h);ctx.fillStyle='#0f1923';ctx.fillRect(0,0,w,h);
        ctx.strokeStyle='rgba(45,74,98,0.3)';ctx.lineWidth=1;
        for(let i=0;i<=4;i++){let y=pt+(ch/4)*i;ctx.beginPath();ctx.moveTo(pl,y);ctx.lineTo(w-pr,y);ctx.stroke();}
        const pts=[];for(let t=1;t<=30;t++){pts.push({t,w:calcWeight(t,document.getElementById('iceType').value).weight});}
        const mw=1600,mt=1,xt=30;
        ctx.beginPath();ctx.moveTo(pl,pt+ch);
        pts.forEach(p=>{let x=pl+((p.t-mt)/(xt-mt))*cw2,y=pt+ch-(p.w/mw)*ch;ctx.lineTo(x,y);});
        ctx.lineTo(pl+cw2,pt+ch);ctx.closePath();
        const ag=ctx.createLinearGradient(0,pt,0,pt+ch);ag.addColorStop(0,'rgba(26,115,232,0.3)');ag.addColorStop(1,'rgba(26,115,232,0.05)');
        ctx.fillStyle=ag;ctx.fill();
        ctx.beginPath();pts.forEach((p,i)=>{let x=pl+((p.t-mt)/(xt-mt))*cw2,y=pt+ch-(p.w/mw)*ch;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});
        ctx.strokeStyle='#1a73e8';ctx.lineWidth=2;ctx.stroke();
        const cx2=pl+((ct-mt)/(xt-mt))*cw2,cy=pt+ch-(cw/mw)*ch;
        ctx.beginPath();ctx.arc(cx2,cy,6,0,Math.PI*2);ctx.fillStyle='#4fc3f7';ctx.shadowColor='rgba(79,195,247,0.6)';ctx.shadowBlur=10;ctx.fill();ctx.shadowBlur=0;
        ctx.fillStyle='#e8eaed';ctx.font='11px "Microsoft YaHei",sans-serif';ctx.textAlign='center';ctx.fillText(cw+'g',cx2,cy-12);
        ctx.fillStyle='#5f6368';ctx.font='10px "Microsoft YaHei",sans-serif';ctx.textAlign='center';
        for(let t=5;t<=30;t+=5){let x=pl+((t-mt)/(xt-mt))*cw2;ctx.fillText(t+'mm',x,h-5);}
        ctx.textAlign='right';for(let w2=0;w2<=1600;w2+=400){let y=pt+ch-(w2/mw)*ch;ctx.fillText(w2+'g',pl-5,y+4);}
        ctx.fillStyle='#9aa0a6';ctx.font='11px "Microsoft YaHei",sans-serif';ctx.textAlign='left';ctx.fillText('冰厚-配重关系曲线',pl,14);
    }

    document.getElementById('thicknessSlider').addEventListener('input',function(){document.getElementById('iceThickness').value=this.value;updWeight();});
    document.getElementById('iceThickness').addEventListener('input',function(){let v=parseInt(this.value)||1;v=Math.max(1,Math.min(30,v));this.value=v;document.getElementById('thicknessSlider').value=v;updWeight();});
    document.getElementById('iceType').addEventListener('change',updWeight);
    document.getElementById('calcWeightBtn').addEventListener('click',updWeight);
    updWeight();
    ...
    const speedData = {};
    for(let i=1;i<=30;i++){
        const s=Math.round(80*Math.pow(0.9,i-1));
        speedData[i]={speed:Math.max(s,3),maxSafe:Math.min(Math.round(s*1.25),100),minWork:Math.max(Math.round(s*0.4),1),efficiency:Math.max(Math.round(100*Math.pow(0.92,i-1)),5),risk:i<=3?'极低':i<=7?'低':i<=14?'中':i<=20?'高':'极高'};
    }

    const weatherFactors={clear:1.0,cloudy:0.9,snowy:0.7,storm:0.5};
    const lineTypeFactors={high:1.0,medium:0.85,low:0.7};

    function calcSpeed(th,weather,line){
        const b=speedData[th]||speedData[5];
        const wf=weatherFactors[weather]||1.0,lf=lineTypeFactors[line]||1.0;
        return {speed:Math.round(b.speed*wf*lf),maxSafe:Math.round(b.maxSafe*wf*lf),minWork:Math.round(b.minWork*wf),efficiency:Math.round(b.efficiency*wf),risk:b.risk,estTime:b.speed>0?Math.round(60/b.speed*10)/10:60};
    }

    function drawSpeedGauge(speed,maxSafe){
        const c=document.getElementById('speedGauge'),ctx=c.getContext('2d');
        const w=c.width,h=c.height,cx=w/2,cy=h/2+20,r=110;
        ctx.clearRect(0,0,w,h);ctx.fillStyle='#0f1923';ctx.fillRect(0,0,w,h);
        const sa=Math.PI*0.75,ea=Math.PI*2.25,ta=ea-sa;
        const segs=[{s:0,e:0.3,c:'#34a853'},{s:0.3,e:0.6,c:'#fbbc04'},{s:0.6,e:0.85,c:'#ea4335'},{s:0.85,e:1.0,c:'#9334e6'}];
        segs.forEach(seg=>{ctx.beginPath();ctx.arc(cx,cy,r,sa+ta*seg.s,sa+ta*seg.e);ctx.strokeStyle=seg.c;ctx.lineWidth=20;ctx.stroke();});
        for(let i=0;i<=10;i++){let a=sa+(ta/10)*i,im=i%2===0,ir=r-(im?15:10),or=r-2;
        ctx.beginPath();ctx.moveTo(cx+Math.cos(a)*ir,cy+Math.sin(a)*ir);ctx.lineTo(cx+Math.cos(a)*or,cy+Math.sin(a)*or);
        ctx.strokeStyle='#5f6368';ctx.lineWidth=im?2:1;ctx.stroke();
        if(im){let lr=r-25;ctx.fillStyle='#9aa0a6';ctx.font='10px "Microsoft YaHei",sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(i*10,cx+Math.cos(a)*lr,cy+Math.sin(a)*lr);}}
        const sr=Math.min(speed/100,1),pa2=sa+ta*sr;
        ctx.shadowColor='rgba(79,195,247,0.3)';ctx.shadowBlur=10;
        ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+Math.cos(pa2)*(r-25),cy+Math.sin(pa2)*(r-25));
        ctx.strokeStyle='#4fc3f7';ctx.lineWidth=3;ctx.stroke();ctx.shadowBlur=0;
        ctx.beginPath();ctx.arc(cx,cy,8,0,Math.PI*2);ctx.fillStyle='#4fc3f7';ctx.fill();
        ctx.beginPath();ctx.arc(cx,cy,4,0,Math.PI*2);ctx.fillStyle='#0f1923';ctx.fill();
        if(maxSafe<100){let sfr=maxSafe/100,sa2=sa+ta*sfr;
        ctx.beginPath();ctx.moveTo(cx+Math.cos(sa2)*(r-30),cy+Math.sin(sa2)*(r-30));ctx.lineTo(cx+Math.cos(sa2)*(r-5),cy+Math.sin(sa2)*(r-5));
        ctx.strokeStyle='#ea4335';ctx.lineWidth=3;ctx.stroke();
        ctx.fillStyle='#ea4335';ctx.font='9px "Microsoft YaHei",sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('上限',cx+Math.cos(sa2)*(r-35),cy+Math.sin(sa2)*(r-35));}
    }

    function updSpeed(){
        const th=parseInt(document.getElementById('speedIceThickness').value);
        const weather=document.getElementById('weatherCondition').value;
        const line=document.getElementById('lineType').value;
        const r=calcSpeed(th,weather,line);
        document.getElementById('speedResult').textContent=r.speed;
        document.getElementById('recSpeed').textContent=r.speed+' km/h';
        document.getElementById('maxSafeSpeed').textContent=r.maxSafe+' km/h';
        document.getElementById('minWorkSpeed').textContent=r.minWork+' km/h';
        document.getElementById('iceEfficiency').textContent=r.efficiency+'%';
        document.getElementById('estTime').textContent=r.estTime+' min/km';
        const el=document.getElementById('riskLevel');
        el.textContent=r.risk;
        el.style.color=r.risk==='极低'||r.risk==='低'?'#34a853':r.risk==='中'?'#fbbc04':r.risk==='高'?'#ea4335':'#9334e6';
        drawSpeedGauge(r.speed,r.maxSafe);
        updSpeedTable(th);
    }

    function updSpeedTable(ht){
        const tb=document.getElementById('speedTableBody');tb.innerHTML='';
        for(let t=1;t<=30;t++){
            const d=speedData[t];const tr=document.createElement('tr');
            if(t===ht){tr.style.background='rgba(26,115,232,0.15)';tr.style.borderLeft='3px solid #1a73e8';}
            const rc=d.risk==='极低'||d.risk==='低'?'#34a853':d.risk==='中'?'#fbbc04':d.risk==='高'?'#ea4335':'#9334e6';
            tr.innerHTML='<td>'+t+' mm</td><td>'+d.speed+' km/h</td><td>'+d.maxSafe+' km/h</td><td>'+d.efficiency+'%</td><td style="color:'+rc+'">'+d.risk+'</td>';
            tb.appendChild(tr);
        }
    }

    document.getElementById('speedThicknessSlider').addEventListener('input',function(){document.getElementById('speedIceThickness').value=this.value;updSpeed();});
    document.getElementById('speedIceThickness').addEventListener('input',function(){let v=parseInt(this.value)||1;v=Math.max(1,Math.min(30,v));this.value=v;document.getElementById('speedThicknessSlider').value=v;updSpeed();});
    document.getElementById('weatherCondition').addEventListener('change',updSpeed);
    document.getElementById('lineType').addEventListener('change',updSpeed);
    updSpeed();
});
